"""Read-only Deribit USDC delivery books under a frozen cost protocol."""
...
PROTOCOL = "docs/deribit_usdc_delivery_universe_protocol.md"
REPORT = ROOT / "outputs/deribit_usdc_delivery_universe.json"
CONTRACTS = (
    "AVAX_USDC-30OCT26", "BTC_USDC-30OCT26", "ETH_USDC-30OCT26",
    "SOL_USDC-30OCT26", "XRP_USDC-30OCT26",
    "BTC_USDC-27NOV26", "ETH_USDC-27NOV26",
    "AVAX_USDC-25DEC26", "BTC_USDC-25DEC26", "ETH_USDC-25DEC26",
    "SOL_USDC-25DEC26", "XRP_USDC-25DEC26",
)
TARGETS = (500, 1000)
...
USDC_INDEX_VALUATION = Decimal("0.0025")
...
def result(read: dict):
    if read.get("status") != "ok":
        raise ValueError("public API request failed")
    payload = read["payload"]
    if payload.get("jsonrpc") != "2.0" or "error" in payload:
        raise ValueError(f"Deribit API error: {payload.get('error')}")
    return payload["result"]


def evaluate(symbol: str, spot: dict, future: dict,
             spot_read: dict, future_read: dict, time_read: dict) -> dict:
    base = symbol.split("_USDC-")[0]
    ...
    if (spot.get("instrument_name") != spot_symbol or not spot.get("is_active")
            ...
            or future.get("quote_currency") != "USDC"
    ...
    for target in TARGETS:
        hedge_coin = round_step(Decimal(target) / best_ask, future_step, ROUND_DOWN)
        spot_coin = round_step(hedge_coin, spot_step, ROUND_UP)
        row = {"intended_spot_notional": target, "hedge_coin": str(hedge_coin),
               "spot_coin": str(spot_coin),
               "excess_spot_coin_written_off": str(spot_coin - hedge_coin)}
        if hedge_coin < future_min or spot_coin < spot_min:
            row["status"] = "below exchange minimum"
        else:
            future_cash, future_visible = book_cash(
                future_book["bids"], hedge_coin, Decimal(1))
            spot_cash, spot_visible = book_cash(
                spot_book["asks"], spot_coin, Decimal(1))
            row["visible_future_coin"] = str(future_visible)
            ...
            if future_cash is None or spot_cash is None:
                ...
                fees = {
                    ...
                    "usdc_index_valuation": USDC_INDEX_VALUATION * future_cash,
...
def main() -> None:
    ...
    report = {"protocol": PROTOCOL, "protocol_commit": commit,
              ...
              "targets": TARGETS, "cost_assumptions": {
                  ...
                  "usdc_index_valuation": str(USDC_INDEX_VALUATION),
              ...
              "limits": "Public books and conditional cash settlement only; actual access, routing, fees, fills, margin and final exit unknown; no orders"}
    spot_meta = read_public(API, "/api/v2/public/get_instruments",
                            {"currency": "USDC", "kind": "spot", "expired": "false"})
    future_meta = read_public(API, "/api/v2/public/get_instruments",
                              {"currency": "USDC", "kind": "future", "expired": "false"})
    report["metadata"] = {"spot": spot_meta, "future": future_meta}
    try:
        spots, futures = result(spot_meta), result(future_meta)
    ...
    for symbol in CONTRACTS:
        ...
        try:
            rowset = evaluate(symbol, spot, future, spot_read, future_read, time_read)
